...
module tb_tanh_all_formats;
    ...
    tanh_top_fp32 dut_fp32 (.clk(clk), .b(b_fp32), .z(z_fp32));
    tanh_top_int8 dut_int8 (.clk(clk), .b(b_int8), .z(z_int8));
    tanh_top_uint8 dut_uint8 (.clk(clk), .b(b_uint8), .z(z_uint8));
    tanh_top_fp8 dut_fp8 (.clk(clk), .b(b_fp8), .z(z_fp8));
    ...
    initial begin
        $display("=============================================================");
        $display("UNIFIED TANH FULL-COVERAGE TESTBENCH - ALL FORMATS");
        $display("=============================================================");
        ...
        for (i = -128; i <= 127; i = i + 1) begin
            ...
            if (abs_err > 1e-5) begin
                ...
                $display("  %+4d  z_float=%e  ref=%e  err=%e", i, z_real, ref_real, abs_err);
            end
        end
        ...
        for (i = 0; i <= 255; i = i + 1) begin
            b_uint8 = i[7:0];
            #10;
            x_real = $itor(i) / 127.5 - 1.0;
            ref_real = (($exp(2.0 * x_real) - 1.0) / ($exp(2.0 * x_real) + 1.0));
            z_real = $itor(z_uint8) / 127.5 - 1.0;
            abs_err = (z_real > ref_real) ? (z_real - ref_real) : (ref_real - z_real);
            sum_err = sum_err + abs_err;
            if (abs_err > max_err) max_err = abs_err;
            if (abs_err > 1e-5) begin
                fail_count = fail_count + 1;
                $display("  %3d  x=%+6.3f  z_float=%e  ref=%e  err=%e", i, x_real, z_real, ref_real, abs_err);
            end
        end
        ...
        print_status("FP32", 0.005, 0.02);

        $display("");
        $display("=============================================================");
        $display("UNIFIED TANH FULL-COVERAGE TESTBENCH COMPLETE");
        $display("=============================================================");
        $finish;
    end
