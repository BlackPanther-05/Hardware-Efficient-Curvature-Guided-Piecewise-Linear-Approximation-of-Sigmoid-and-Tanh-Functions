# Run one registered activation implementation on the Virtex-7 device.
#
...
set xdc_template [file join $flow_dir constraints virtex7_benchmark.xdc]
...
close $xdc_in
regsub -all {period 1\.667} $xdc_text "period $period_ns" xdc_text
set half_period [expr {$period_ns / 2.0}]
regsub -all {\{0\.000 0\.833\}} $xdc_text [format "{0.000 %.3f}" $half_period] xdc_text
set xdc_out [open $generated_xdc w]
puts -nonewline $xdc_out $xdc_text
close $xdc_out

set data_width [expr {$data_format eq "fp32" ? 32 : 8}]
set part_name xc7v585tffg1157-3
...
if {[file exists [file join $reports_dir post_route_utilization.rpt]]} {
    ...
    close $fp
    regexp {Slice\s*\|\s*(\d+)} $util_data -> slices
}
